...
public class ServiceConfiguration {
     ...
     */
    public static final File CRM_STORAGE_DIRECTORY = new File(System.getProperty("user.home"), CRM_NAME);

    /**
     * Things are first uploaded by the application server to this directory (staging).
     */
        ...
        for (File file : files) {
            if (!file.exists() && !file.mkdirs()) {
                String msg = String.format("You must create the profile photos directory ('%s') " +
                    "and make it accessible to this process. Unable to do so from this process.", file.getAbsolutePath());
    ...
    static class DefaultDataSourceConfiguration {
        ...
        protected void setupTestProfileImages() throws Exception {
            ...
            if (!profilePhotoForUser5.exists()) {
                ...
                log.debug("Setup photo " + profilePhotoForUser5.getAbsolutePath() + " for the sample user #" + Long.toString(userId));
            }
            if (!profilePhotoForUser5.exists()) {
                throw new RuntimeException("Couldn't setup up profile photos at " + profilePhotoForUser5.getAbsolutePath());
            }
        }
